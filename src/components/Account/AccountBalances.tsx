...
export default (props: { dao: IDAOState, address: Address}) => {
    ...
    return <Subscribe observable={observable}>{(state: IObservableState<[IMemberState, BN, BN, any]>) => {
        if (state.isLoading) {
          ...
          return <AccountBalances dao={props.dao} member={state.data[0]} ethBalance={new BN(state.data[1])} genBalance={state.data[2]} genAllowance={state.data[3] ? new BN(state.data[3].amount) : new BN(0)} />;
        }
      }
    }</Subscribe>;
};
