...
const arcjs_network = Arc.Config.get('arcjs_network');

if (process.env.NODE_ENV == 'production') {
  // Use Infura on production
  const infuraNetwork = arcjs_network == 'live' ? 'mainnet' : arcjs_network;
...
const cacheBlockchain = async () => {
  ...
  const s3 = new aws.S3();
  const fileName = 'initialArcState-' + arcjs_network + '.json';
  ...
  s3.putObject(s3Params, (err, data) => {
    if (err) {
      ...
      // tslint:disable-next-line:no-console
      console.log("Successfully wrote cached data for " + arcjs_network + " to S3. ", data);
    }
  });
